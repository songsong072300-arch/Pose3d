...
def main():
    ...
    info=json.load(open(args.model_points)); i=info.get('1',info.get(1)); sx,sy,sz=[i[k] for k in ('size_x','size_y','size_z')]; obj=np.array([[-sx/2,-sy/2,-sz/2],[-sx/2,-sy/2,sz/2],[-sx/2,sy/2,-sz/2],[-sx/2,sy/2,sz/2],[sx/2,-sy/2,-sz/2],[sx/2,-sy/2,sz/2],[sx/2,sy/2,-sz/2],[sx/2,sy/2,sz/2]],np.float32)
